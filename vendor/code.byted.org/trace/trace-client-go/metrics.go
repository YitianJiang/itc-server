...
const (
	TRACE_CLIENT_NAMESPACE = "toutiao.infra.trace.client.go"
)
...
func (mf *bytedMetricsFactory) Namespace(name string, tags map[string]string) jm.Factory {
	...
	ctags := make([]bm.T, 0, len(tags))
	for k, v := range tags {
		ctags = append(ctags, bm.Tag(k, v))
	}
	return &bytedMetricsFactory{mc: nmc, tags: ctags}
}
