...
func (s *Stats) EmitLoop(d time.Duration, ets ...EmitType) {
	...
	for range time.Tick(d) {
		...
		if et&EmitGoAllocObjectsCounter != 0 && m1.Mallocs-m0.Mallocs > 0 {
			s.mcli.EmitCounter("runtime.alloc.objects", m1.Mallocs-m0.Mallocs)
		}
		if et&EmitGoAllocBytesCounter != 0 && m1.TotalAlloc-m0.TotalAlloc > 0 {
			s.mcli.EmitCounter("runtime.alloc.bytes", m1.TotalAlloc-m0.TotalAlloc)
		}
		if et&EmitGoGCPause != 0 {
			s.mcli.EmitTimer("runtime.gc.pause", GCPauseNs(m1, m0))
		}
		if et&EmitGoGCNum != 0 && m1.NumGC-m0.NumGC > 0 {
			s.mcli.EmitCounter("runtime.gc.count", m1.NumGC-m0.NumGC)
			s.mcli.EmitCounter("runtime.gc.num", m1.NumGC-m0.NumGC) // FIXME: to be deleted in future
		}
		if et&EmitGoroutineNum != 0 {
			s.mcli.EmitStore("runtime.goroutine.num", runtime.NumGoroutine())
		}
		if et&EmitCgoCall != 0 {
			calls := runtime.NumCgoCall()
			if n := calls - lastCgoCalls; n > 0 {
				s.mcli.EmitCounter("runtime.cgocall.count", n)
			}
			lastCgoCalls = calls
		}
		if et&EmitCGroupCPUStat != 0 {
			...
			if n := s1.NrThrottled - s0.NrThrottled; n > 0 {
				s.mcli.EmitCounter("cgroup.cpustat.nr_throttled", n)
			}
			if n := s1.NrPeriods - s0.NrPeriods; n > 0 {
				s.mcli.EmitCounter("cgroup.cpustat.nr_periods", n)
			}
			if n := s1.CPUTime - s0.CPUTime; n > 0 {
				s.mcli.EmitCounter("cgroup.cputime", n)
			}
		}
		m0 = m1
		s0 = s1
