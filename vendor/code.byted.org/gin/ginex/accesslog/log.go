...
func AccessLog(logger *logs.Logger) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		if stressTag == nil {
			...
			if kvLogger == nil {
				logger.Trace("%s %s %s %s status=%d cost=%d method=%s full_path=%s client_ip=%s host=%s stress_tag=%s",
					localIp, psm, c.Value(internal.LOGIDKEY), cluster, c.Writer.Status(), latency, c.Request.Method, path, c.ClientIP(), c.Request.Host, stressTag)
			} else {
				kvs := []interface{}{"status", c.Writer.Status(),
					"cost", latency,
					"method", c.Request.Method,
