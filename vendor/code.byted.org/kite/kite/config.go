...
func initFromConfFile() {
	...
	if env.IsProduct() {
		...
		item := GetConfigItem(cfg, "Testing")
		if item != nil {
			ServiceConfig = item
		} else {
			fmt.Fprintln(os.Stderr, "no Testing-env config, Develop-env config will be used")
	...
	EnableTracing = ServiceConfig.DefaultBool("EnableTracing", false)
