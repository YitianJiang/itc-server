				...
				for _,keyInfo := range keys[0:len(keys)-1] {
					...
					if v,ok := strIgs[keyInfo]; ok{
						...
						if info["status"] != 0 {
							...
							if confirmInfo.Status == 2 && passFlag{
								passFlag = false
							}
						}
					} else{
...
/**
	重组任务的权限检测结果
 */
func GetTaskPermissions(taskId string,appId int,index ...int) (*PermSlice,error) {
	var realIndex int
	if len(index) == 0 {
		realIndex = 0
	}else {
