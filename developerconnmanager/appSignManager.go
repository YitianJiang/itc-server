...
func GetAppSignListDetailInfo(c *gin.Context) {
	...
	sql := "select aac.app_name,aac.app_type,aac.id as app_acount_id,aac.team_id,aac.account_verify_status,aac.account_verify_user," +
		...
		" where acc.app_id = '" + requestInfo.AppId + "' and aac.deleted IS NULL and (aac.dev_cert_id = ac.id or aac.dist_cert_id = ac.id) and ac.deleted_at IS NULL "
	f_query := devconnmanager.QueryWithSql(sql, &fQueryResult)

	var resourcPerm devconnmanager.GetPermsResponse
	url := _const.Certain_Resource_All_PERMS_URL + "employeeKey=" + requestInfo.Username + "&resourceKeys[]=" + ""
